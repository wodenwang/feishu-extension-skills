from feishu_extension_skills.runtime.actions import FEISHU_CHAT_DELETE
...
def test_dispatcher_lazy_loads_delete_action(monkeypatch) -> None:
    def fake_delete_chat_action(args, service=None):
        return {"chat_id": args["chat_id"], "status": "deleted"}

    monkeypatch.setattr(im_chat_actions, "delete_chat_action", fake_delete_chat_action)

    dispatcher = ActionDispatcher(ActionRegistry())
    result = dispatcher.dispatch(
        InvokeRequest(
            action=FEISHU_CHAT_DELETE,
            args={
                "app_id": "cli_xxx",
                "app_secret": "sec_xxx",
                "chat_id": "oc_123",
    ...
    assert result.ok is True
    assert result.data == {"chat_id": "oc_123", "status": "deleted"}
