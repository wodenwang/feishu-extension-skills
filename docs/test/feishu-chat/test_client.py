...
from feishu_extension_skills.services.im_chat.models import AddChatMembersInput, CreateChatInput, DisbandChatInput, ListChatMembersInput, RemoveChatMemberInput
...
def test_client_get_chat_uses_requested_chat_id_when_response_omits_it(monkeypatch: pytest.MonkeyPatch) -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        assert request.method == "GET"
        assert request.url.path.endswith("/im/v1/chats/oc_123")
        return httpx.Response(
            200,
            json={
                "code": 0,
    ...
    result = client.get_chat({"app_id": "cli_xxx", "app_secret": "sec_xxx"}, "oc_123")

    assert result.chat_id == "oc_123"
    assert result.name == "项目群"
    assert result.member_count == 3
    assert result.avatar_url == "https://example.com/avatar.png"
    assert result.raw["user_count"] == "3"


def test_client_disband_chat_hits_delete_endpoint(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = client.disband_chat(
        DisbandChatInput.model_validate(
            {
                "app_id": "cli_xxx",
                "app_secret": "sec_xxx",
                "chat_id": "oc_123",
